...
bool prompt_large_output_if_needed(const std::uint64_t estimated_total_bytes, const std::uint64_t threshold_bytes) {
...
OutputStats FileBackend::generate(const config::GenerationConfig& cfg, const engine::ExecutionOptions& options) {
    ...
    const engine::GenerateResult result =
        engine::generate_with_consumer(cfg, options, [&](engine::Row&& row, std::uint64_t) {
            if (format ==
                config::OutputFormat::Csv ||
                format ==
                config::OutputFormat::TabDelimited ||
